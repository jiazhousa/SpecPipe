...
草案的使命是与用户**快速对齐**需求的两侧：输入侧（背景 / 目标 / 范围）与输出侧（验收标准初版）。验收标准是人对需求可验证度的锚点——须在草案即呈现（初版即可，逐条可核验），不得留待终稿；用户对齐"做什么"的同时必须能对齐"做到什么程度算数"。澄清访谈的载体是提问工具（结构化问答），草案不是访谈道具。

## S-S4 访谈与澄清（用户参与）

前置访谈已完成设想与范围澄清，此处针对 **spec 草案中的业务细节待澄清项**做专项访谈（载体为提问工具，每次 1-3 个问题）——确认单个 Story 的所有业务规则、验收标准、边界条件；澄清答案落入验收标准与业务规则，逐轮更新 spec。澄清完毕后产出 spec 终稿（≤300 行），终稿骨架见 `templates/spec-template.md`。
