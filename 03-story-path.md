...
**无法澄清时的出口（硬门）**：若待澄清项属于业务逻辑本身未定（非技术实现问题），禁止调度者脑补业务决策向下推进——Agent 是业务逻辑的执行层，对业务决策**有建议权、无决策权**。此时仅有两条出口，且均须用户显式确认：

1. **假设落盘**：调度者提议业务假设（写入业务规则节，标注「假设」与验证锚点），经用户确认后按假设推进——不确定性转化为用户显式承担的假设；
2. **探针拆分**：将「想清楚业务」本身立为最小 Story（调研/原型/数据验证），结论产出后再立正式 Story。

待澄清未清零（或未按上述出口显式转化）时，不得产出终稿、不得进入 S-S5。
